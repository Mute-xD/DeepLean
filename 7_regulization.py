...
class MNIST:
    def __init__(self):
        ...
        self.reg = k.regularizers.l2(.001)
    ...
    def build(self):
        self.model.add(k.layers.Dense(200, input_dim=784, activation='tanh', kernel_regularizer=self.reg))
        self.model.add(k.layers.Dense(100, 'tanh', kernel_regularizer=self.reg))
        self.model.add(k.layers.Dense(10, 'softmax', kernel_regularizer=self.reg))
        self.model.compile(optimizer=self.opt, loss='categorical_crossentropy', metrics=['accuracy'])
        self.model.fit(self.xTrain, self.yTrain, batch_size=32, epochs=20)
